    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(startDate.compareTo(endDate) > 0) {
                request.setAttribute("error", "Start date must before end date");
                request.getRequestDispatcher("UserOrder.jsp").forward(request, response);
            } else {
                ArrayList<Order> list = OrderDao.getOrders(email, startDate, endDate);
                request.setAttribute("listOrderByDate", list);
                request.getRequestDispatcher("UserOrder.jsp").forward(request, response);
            }
        } catch (Exception e) {
            e.printStackTrace();
